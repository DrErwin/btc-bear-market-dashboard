...
> 状态：核心产品决策与书面规格已于 2026-07-29 获用户确认；v0.4.0 已在本地完整实现并通过统一自动验收。本文记录设计与实施进度；真实模型认证、线上部署和数据授权变更仍需单独批准。

## 当前进度

- 已完成：双轴市场状态、证据职责、人工校准档位保护、前三天上下文、修复与离开窗口、AI 与机器边界及页面解释结构已经确认。
- 当前状态：完整实施规格已完成自审，并发布为 [GitHub Issue #2](https://github.com/DrErwin/btc-bear-market-dashboard/issues/2)，标签为 `ready-for-agent`；用户已确认进入实施并完成本地验收。
- 已完成本阶段：完成实施前状态审计，确认旧单阶段代码、当前验收入口、完整结果包边界，以及工作区中与本任务无关的已有改动；现版本已作为基线提交 `c05c4e3`。
- 已完成阶段 1：双轴契约、稳定档位字段、代表性结果包与数据不足/回退/非法输出夹具已冻结，旧单阶段输入会被拒绝。
- 已完成阶段 2：16 项指标已迁移到稳定档位身份、单一主要职责和相关性家族；阈值数值、方向、顺序与人工校准含义未改变。
- 已完成阶段 3：时间线、730 日窗口验证记录、修复/反转事实和前三个自然日上下文已实现。
- 已完成阶段 4：AI 双轴输入、六段详细解释、实际值/方向/日期/可用性语义校验、每轴数据不足和 v0.4 回退已实现。
- 已完成阶段 5：公开页面已替换为双轴结论，保留指标卡片与图表，并使用稳定档位 ID 驱动颜色。
- 已完成阶段 6a：版本索引、README、v0.3.1 历史迁移说明、v0.4.0 实现记录、验收记录和 730 日窗口验证记录已补齐。
- 已完成阶段 6b：完成标准与规格双轴审查，并修复数据不足强制、状态变化必填、稳定档位独立配置、过期时间线、AI 失败回退和旧维护脚本等问题。
- 已完成集成复验：统一入口通过，包含 73 个 Python 测试、前端构建、success／partial／both-axis-insufficient／fallback／no-fallback、键盘和 390px 移动端页面场景。
- 已完成阶段 6c：完成最终回归验收，`73` 个 Python 测试、前端构建和统一浏览器验收全部通过。
- 已完成阶段 6d：v0.4.0 本地实现已提交，工作区已恢复干净。
- 已完成阶段 6e：根据浏览器批注完成 UI 收敛：隐藏过期展示提示和柱状系列说明，双轴状态卡改为坐标轴节点样式，并补充对应浏览器验收断言。
- 已完成阶段 6f：根据浏览器批注将 CVDD 调整为当前可用，移除验证提示，重新生成结果包并补充页面断言；它仍只作为长期锚点背景，不单独决定状态。
- 当前状态：本地实施与 UI 收敛阶段完成；真实模型评估、线上部署和数据授权变更仍需单独批准。
- 待办：进入独立的真实模型与线上发布评估，不自动改变当前运行契约。
...
| CVDD 接近程度 | 长期锚点 | 长期底部接近背景 | 当前可用，但只作为长期锚点背景，不单独决定状态。 |
...
- v0.4.0 已在本地实现并验收；线上发布仍需单独批准，历史 v0.3 契约不再进入当前运行链路；
- 历史 `allowed_stages` 只保留在版本记录和迁移说明中，当前产品约束统一采用双轴状态词汇和数据边界。
...
本地实现和固定验收已经完成，后续工作不再改变已确认的双轴产品边界，除非用户另行批准：

1. 使用真实模型进行提示词、返回质量、拒答和长周期稳定性评估；
2. 复核公开缓存、静态包和第三方数据的再分发授权；
3. 在独立的线上发布检查通过后再部署 v0.4.0；
4. 若真实数据或模型暴露新问题，先补充固定夹具和验收，再修改运行契约。
