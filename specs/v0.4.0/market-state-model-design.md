...
> 状态：核心产品决策与书面规格已于 2026-07-29 获用户确认；用户已要求完成 0.4.0 全量实施，现进入实施阶段。本文记录设计与实施进度；运行逻辑正在按下方规格迁移，尚未代表线上发布。

## 当前进度

- 已完成：双轴市场状态、证据职责、人工校准档位保护、前三天上下文、修复与离开窗口、AI 与机器边界及页面解释结构已经确认。
- 当前状态：完整实施规格已完成自审，并发布为 [GitHub Issue #2](https://github.com/DrErwin/btc-bear-market-dashboard/issues/2)，标签为 `ready-for-agent`；用户已确认进入实施。
- 已完成本阶段：完成实施前状态审计，确认旧单阶段代码、当前验收入口、完整结果包边界，以及工作区中与本任务无关的已有改动。
- 当前阶段：阶段 1“契约与固定验收场景”进行中。
- 待办：冻结双轴契约、稳定档位身份、代表性成功/数据不足/回退/非法输出夹具，并让统一验收入口先表达新版行为。
- 下一步：契约通过后进入档位与 16 项指标职责迁移，再实现时间线、AI、结果包和页面。
